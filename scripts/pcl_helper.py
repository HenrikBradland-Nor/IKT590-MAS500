...
def rgb_to_float(color):
    ...
    """
    hex_r = (0xff & color[0]) << 24
    hex_g = (0xff & color[1]) << 16
    hex_b = (0xff & color[2]) << 8

    hex_rgb = hex_r | hex_g | hex_b | 0x00
...
def float_to_rgb_hex_original(float_rgb):
    s = struct.pack('>f', float_rgb)
    i = struct.unpack('>l', s)[0]
    pack = ctypes.c_uint32(i).value
    ...
    return struct.unpack('f', struct.pack('i', color))[0]


def float_to_rgb_hex(float_rgb):
    s = struct.pack('<f', float_rgb)
    r, g, b, a = struct.unpack('BBBB', s)

    #print("a", a, "r", r, "g", g, "b", b)
    hex_num = ctypes.c_uint32((a << 24)|(b << 16) | (g << 8) | r)
    flo_2 = struct.pack('I', hex_num.value)
    # flo_3 = struct.pack('l', hex_num)[4:]
    # print(hex_num.value)
    # print(type(hex_num))
...
def test(float_rgb):
    ...
    pack = ctypes.c_uint32(i).value
    #a, b, g, r = struct.unpack('>BBBB', s)

    a = (pack & 0xFF000000) >> 24
    b = (pack & 0x00FF0000) >> 16
    g = (pack & 0x0000FF00) >> 8
    r = (pack & 0x000000FF)
